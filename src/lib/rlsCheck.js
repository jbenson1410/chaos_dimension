...
export async function checkRlsState(db, { checkRole = true } = {}) {
  const problems = [];
  ...
  //    This is only meaningful on the RUNTIME (cd_app) connection. The
  //    migration script connects as the table owner, which on Neon is always
  //    a BYPASSRLS superuser-family role (neondb_owner) — so it would always
  //    false-positive here. The migration passes { checkRole: false } and the
  //    runtime role is policed instead by `npm run db:verify-rls`, which runs
  //    as cd_app via getDb().
  if (checkRole) {
...
export async function assertRlsState(db, opts) {
  const problems = await checkRlsState(db, opts);
  if (problems.length) {
